**Page 1 / Network Traffic Analysis**
...
7. [`Span Ports`](https://en.wikipedia.org/wiki/Port_mirroring): are a way to copy frames from layer two or three networking devices during egress or ingress processing and send them to a collection point. Often a port is mirrored to send those copies to a log server.
...
A. tcpdump -i eth0 -nvXw /tmp/output.pcap
...
**Page 8 / Tcpdump Packet Filtering**
...
```shell-session
W4H33D@htb[/htb]$ sudo tcpdump -i eth0 greater 500
...
```shell-session
W4H33D@htb[/htb]$ ### Syntax: and [requirement]
W4H33D@htb[/htb]$ sudo tcpdump -i eth0 host 192.168.0.1 and port 23
...
```shell-session
W4H33D@htb[/htb]$ ### Syntax: or/|| [requirement]
W4H33D@htb[/htb]$ sudo tcpdump -r sus.pcap icmp or host 172.16.146.1
...
```shell-session
W4H33D@htb[/htb]$ ### Syntax: not/! [requirement]
W4H33D@htb[/htb]$ sudo tcpdump -r sus.pcap not icmp
...
```shell-session
W4H33D@htb[/htb]$sudo tcpdump -Ar telnet.pcap
...
```shell-session
W4H33D@htb[/htb]$ sudo tcpdump -Ar http.cap -l | grep 'mailto:*'
...
```shell-session
W4H33D@htb[/htb]$ tcpdump -i eth0 'tcp[13] & 2 != 0'
```
...
```shell-session
W4H33D@htb[/htb]$ sudo tcpdump -i eth0 'tcp[13] &2 != 0'
...
Link                                          Description
```
1. [IP Protocol](https://tools.ietf.org/html/rfc791): `RFC 791` describes IP and its functionality.

2. [ICMP Protocol](https://tools.ietf.org/html/rfc792): `RFC 792` describes ICMP and its functionality.

3. [TCP Protocol](https://tools.ietf.org/html/rfc793): `RFC 793` describes the TCP protocol and how it functions.

4. [UDP Protocol](https://tools.ietf.org/html/rfc768): `RFC 768` describes UDP and how it operates.

5. [RFC Quick Links](https://en.wikipedia.org/wiki/List_of_RFCs#Topical_list): This Wikipedia article contains a large list of protocols tied to the RFC that explains their implementation.

#### Questions

Q1: What filter will allow me to see only traffic coming from the host with an ip of 10.10.20.1?
A: host 10.10.20.1

Q2: What filter will allow me to capture based on either of two options?
A: Or

Q3: True or False: TCPDump will resolve IPs to hostnames by default.
A: True

**Page 9 / Interrogating Network Traffic With Capture and Display Filters**
...
```
$ tcpdump -r TCPDump-lab-2.pcap proto udp or port 53
```
```
...
Q1: What are the client and server port numbers used in first full TCP three-way handshake? (low number first then high number)
Hint: Look for tcp traffic or web traffic. you will notice the client using a large number and the server using a well known port for HTTP.
A: 80 43804

Q2: Based on the traffic seen in the pcap file, who is the DNS server in this network segment? (ip address)
Hint: look at traffic with a source port of 53.
A: 172.16.146.1

**Page 10 / Analysis with Wireshark**
...
```shell-session
W4H33D@htb[/htb]$ which wireshark
```
...
```shell-session
W4H33D@htb[/htb]$ sudo apt install wireshark 
```
...
1. `D`    `Will display any interfaces available to capture from and then exit out.`


2. `L`    `Will list the Link-layer mediums you can capture from and then exit out.  (ethernet as an example)`

3. `i`  `choose an interface to capture from. (-i eth0)`

4. `f` `packet filter in libpcap syntax. Used during capture.`

5. `c` `Grab a specific number of packets, then quit the program. Defines a stop condition.`

6. `a` `Defines an autostop condition. Can be after a duration, specific file size, or after a certain number of packets.`

7. `r (pcap-file)` `Read from a file.`

8. `W (pcap-file)` `Write into a file using the pcapng format.`

9. `P` `Will print the packet summary while writing into a file (-W)`

10. `x` `will add Hex and ASCII output into the capture.`

11. `h` `See the help menu`
...
```shell-session
W4H33D@htb[/htb]$ which tshark

W4H33D@htb[/htb]$ tshark -D

W4H33D@htb[/htb]$ tshark -i 1 -w /tmp/test.pcap
...
```shell-session
W4H33D@htb[/htb]$ sudo tshark -i eth0 -w /tmp/test.pcap
```
...
```shell-session
W4H33D@htb[/htb]$ sudo tshark -i eth0 -f "host 172.16.146.2"
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/termshark.png)
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/termshark-help.png)
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/wireshark-interface.txt)
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/wireshark-menu.txt)
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/first-capture-ws.txt)
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/wireshark-toolbar.jpg)
...
1. `host x.x.x.x` `Capture only traffic pertaining to a certain host`

2. `net x.x.x.x/24` `Capture traffic to or from a specific network (using slash notation to specify the mask)`

3. `src/dst net x.x.x.x/24` `Using src or dst net will only capture traffic sourcing from the specified network or destined to the target network`

4. `port #` `will filter out all traffic except the port you specify`

5. `not port #` `will capture everything except the port specified`

6. `port # and #` `AND will concatenate your specified ports`

7. `portrange x-x` `portrange will grab traffic from all ports within the range only`

8. `ip / ether / tcp` `These filters will only grab traffic from specified protocol headers`

9. `broadcast / multicast / unicast` `Grabs a specific type of traffic. one to one, one to many, or one to all`
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/capture-filter-list.png)
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/how-to-add-cap.png)
...
```
1. `ip.addr == x.x.x.x` `Capture only traffic pertaining to a certain host. This is an OR statement`

2. `ip.addr == x.x.x.x/24` `Capture traffic pertaining to a specific network. This is an OR statement`

3. `ip.src/dst == x.x.x.x` `Capture traffic to or from a specific host`

4. `dns / tcp / ftp / arp / ip` `filter traffic by a specific protocol. There are many more options`

5. `tcp.port == x` `filter by a specific tcp port`.

6. `src.port / dst.port ==x` `will capture everything except the port specified`

7. `and / or / not` `AND will concatenate, OR will find either of two options, NOT will exclude your input option`.
...
![image](https://academy.hackthebox.eu/module/storage/modules/81/display-filter.png)
...
Hint: Wireshark is FOSS, and capable of running on many different Operating Systems
A: True

Q2: Which Pane allows a user to see a summary of each packet grabbed during the capture?
Hint: There are three panes; Packet l__ , Packet b___ , and Packet d______ 
A: Packet List

Q3: Which pane provides you insight into the traffic you captured and displays it in both ASCII and Hex?
A: Packet Bytes

Q4: What switch is used with TShark to list possible interfaces to capture on?
Hint: Similar to TCPDump
A: -D

Q5: What switch allows us to apply filters in TShark?
Hint: this is a unique Filter switch for TShark.
A: -f

Q6:  Is a capture filter applied before the capture starts or after? (answer before or after)
Hint: its to capture, not display
A: before

**Page 11 / Familiarity With Wireshark**
...
![image](https://academy.hackthebox.eu/storage/modules/81/wireshark-statistics.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/analyze.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/follow-tcp.txt)
...
![image](https://academy.hackthebox.eu/storage/modules/81/tcp-stream.txt)
...
![image](https://academy.hackthebox.eu/storage/modules/81/extract-http.txt)
...
![image](https://academy.hackthebox.eu/storage/modules/81/ftp-disector.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/ftp-request-command.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/ftp-data.png)
...
Q1: Which plugin tab can provide us with a way to view conversation metadata and even protocol breakdowns for the entire PCAP file?
Hint: just the word. We are looking for Stats right?
A: Statistics

Q2:  What plugin tab will allow me to accomplish tasks such as applying filters, following streams, and viewing expert info?
Hint: not statistics. we are performing analysis..
A: Analyze

Q3: What stream oriented Transport protocol enables us to follow and rebuild conversations and the included data?
Hint: not UDP
A: TCP

Q4: True or False: Wireshark can extract files from HTTP traffic.
Hint: http uses TCP for transport, which means each byte is accounted for..
A: True

Q5: True or False: The ftp-data filter will show us any data sent over TCP port 21.
Hint: ftp uses tcp port 21 for command control of FTP. What other port does FTP use?
A: False

**Page 13 / Packet Inception, Dissecting Network Traffic With Wireshark**
...
![image](https://academy.hackthebox.eu/storage/modules/81/nomachine-setup.png)
...
Q1: What was the filename of the image that contained a certain Transformer Leader? (name.filetype)
Hint: You should have seen three image files pulled in the traffic. water.jpg, htb.jpeg, and ..?
A: Rise-Up.jpg

Q2: Which employee is suspected of performing potentially malicious actions in the live environment?
Hint: Take a peek at all protocols to get an idea of who.
A: bob

**Page 14 / Guided Lab: Traffic Analysis Workflow**
...
![image](https://academy.hackthebox.eu/storage/modules/81/guided-conversations.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/guided-proto.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/guided-udp.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/guided-tcp.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/guided-handshake.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/guided-stream.png)
...
Q1: what was the name of the new user created on Mr. Ben's host?
Hint: its in the pcap data. Look at the TCP conversation.
A: hacker

Q2: How many total packets were there in the Guided-analysis PCAP?
Hint: total count is shown in the wireshark GUI.
A: 44

Q3: What was the suspicious port that was being used?
Hint: meterpreter loves this port.
A: 4444

**Page 15 / Decrypting RDP connections**
...
![image](https://academy.hackthebox.eu/storage/modules/81/enc-rdp.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/3389.png)
...
2.  On the TLS page, select Edit by RSA keys list → a new window will open. ![image](https://academy.hackthebox.eu/storage/modules/81/import-ws.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/import-steps.png)

When filtering once again on RDP, we should see some traffic in the display.

#### RDP In The Clear

![image](https://academy.hackthebox.eu/storage/modules/81/rdp-clear.png)
...
![image](https://academy.hackthebox.eu/storage/modules/81/rdp-user.png)
...
Hint: His name can be seen when you look for the cookie in ASCII.
A: bucky
